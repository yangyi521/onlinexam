    ...
    <div class="search">
        <form id="form1" method="post" action="stu_Find_findPastExam.action?currentPage=0">
            ...
            <s:iterator value="#session.cousss" var="cou">
            	<s:if test='%{#session.select==#cou.cou_id}'>
                	<option value='<s:property value="#cou.cou_id" />' selected="selected"><s:property value="#cou.cou_name" /></option>
            	</s:if>
            	<s:else>
...
<s:iterator value="#session.stuTestSimpleView" var="stuTest">
  <tr align="center">
    ...
    <td><s:property value="#stuTest.stu_test_time" />分钟</td>
    <td>已完成</td>
...
<table width="90%" border="0" align="center" cellpadding="0"
		...
		<c:choose>
			<c:when test="${page.hasPrePage}">
				<a href="stu_Find_findPastExam.action?currentPage=1">首页</a> | 
				<a href="stu_Find_findPastExam.action?currentPage=${page.currentPage -1 }">上一页</a>
			</c:when>
			<c:otherwise>
							首页 | 上一页
						</c:otherwise>
		...
		<c:choose>
			<c:when test="${page.hasNextPage}">
				<a
					href="stu_Find_findPastExam.action?currentPage=${page.currentPage + 1 }">下一页</a> | 
							<a
					href="stu_Find_findPastExam.action?currentPage=${page.totalPage }">尾页</a>
			</c:when>
			<c:otherwise>
						下一页 | 尾页
					</c:otherwise>
