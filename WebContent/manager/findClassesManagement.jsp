...
<table class="word_darkGrey" width="90%" border="0" align="center" cellpadding="0"
		...
		<c:choose>
			<c:when test="${page.hasPrePage}">
				<a href="sys_Find_findClassesManagement.action?currentPage=1">首页</a> | 
				<a href="sys_Find_findClassesManagement.action?currentPage=${page.currentPage -1 }">上一页</a>
			</c:when>
			<c:otherwise>
							首页 | 上一页
						</c:otherwise>
		...
		<c:choose>
			<c:when test="${page.hasNextPage}">
				<a
					href="sys_Find_findClassesManagement.action?currentPage=${page.currentPage + 1 }">下一页</a> | 
							<a
					href="sys_Find_findClassesManagement.action?currentPage=${page.totalPage }">尾页</a>
			</c:when>
			<c:otherwise>
						下一页 | 尾页
					</c:otherwise>
