  ...
  <div class="search">
        <form id="form1" method="post" action="sys_Find_findTeaManagement.action?currentPage=0">
            <select name="selectk">
                ...
                <s:elseif test='%{#session.selectk.equals("teaId")}'>
                	<option value="courseName">课程科目</option>
	                <option selected="selected" value="teaId">教师编号</option>
	                <option value="teaName">教师姓名</option>
                </s:elseif>
                <s:else>
                	<option value="courseName">课程科目</option>
	                <option value="teaId">教师编号</option>
	                <option selected="selected" value="teaName">教师姓名</option>
                </s:else>
            </select>
            <input type="text" name="name" value='<s:property value="#session.quesname" />' />
...
<table width="90%" border="0" align="center" cellpadding="0"
		...
		<c:choose>
			<c:when test="${page.hasPrePage}">
				<a href="sys_Find_findTeaManagement.action?currentPage=1">首页</a> | 
				<a href="sys_Find_findTeaManagement.action?currentPage=${page.currentPage -1 }">上一页</a>
			</c:when>
			<c:otherwise>
							首页 | 上一页
						</c:otherwise>
		...
		<c:choose>
			<c:when test="${page.hasNextPage}">
				<a
					href="sys_Find_findTeaManagement.action?currentPage=${page.currentPage + 1 }">下一页</a> | 
							<a
					href="sys_Find_findTeaManagement.action?currentPage=${page.totalPage }">尾页</a>
			</c:when>
			<c:otherwise>
						下一页 | 尾页
					</c:otherwise>
